...
test_dataset = dataset.DINCAEDataset(
    data_dir=test_data_dir, transform=train_transform, target_transform=target_transform, use_elevation=use_elevation)
val_dataloader = dataloader.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, drop_last=True)
...
# Tensorboard设置
writer = SummaryWriter("model/" + folder_name + "/tensorboard")

# 开始训练
min_val_loss = 999.90

for epoch in range(251):  # loop over the dataset multiple times
    ...
    if epoch % 5 == 4:
        ...
        if val_loss < min_val_loss:
            ...
            torch.save(coarse_net, "model/" +folder_name + "/coarse_" + str(epoch) + '.pth')
            torch.save(refine_net, "model/" +folder_name + "/refine_" + str(epoch) + '.pth')
