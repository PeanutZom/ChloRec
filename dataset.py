...
class DINCAEDataset(Dataset):
    def __init__(self, data_dir=None, transform=None, target_transform=None):
        """
    ...
    def __getitem__(self, item):
        ...
        with xr.open_dataset(os.path.join(sample_dir, 'cur.nc')) as cur:
            cur = cur
        with xr.open_dataset(os.path.join(sample_dir, 'prev.nc')) as next:
            next = next
        ...
        input_np[2] = cur.CHL1_mean
        input_np[3] = np.round(cur.CHL1_flags % 16 >= 8)
        input_np[4] = np.expand_dims(cur.lat, 1)
        input_np[5] = np.expand_dims(cur.lon, 0)
        ...
        if self.transform is not None:
            input_ts = torch.cat([self.transform(input_ts[0:3]), input_ts[3:]], 0) * missing_mask_ts

        if self.target_transform is not None:
            target_ts = self.target_transform(target_ts) * missing_mask_ts
